...
from skimage.morphology import binary_opening as skimage_binary_opening
...
def create_histogram_based_foreground_mask(
    image_data: np.ndarray,
    otsu_scale_factor: float = 1.0,
    closing_iters: int = 2, # Changed parameter name for clarity from 'iterations'
    opening_iters: int = 1, # Changed parameter name for clarity
    min_object_size_frac: float = 0.1,
    min_hole_size_frac: float = 0.01
    ) -> np.ndarray:
    """
    Creates a binary foreground mask from image data using histogram-based thresholding (Otsu)
    and morphological post-processing. Manually iterates closing/opening operations
    for broader scikit-image version compatibility.

    Args:
        image_data (np.ndarray): The input 2D or 3D image data.
        otsu_scale_factor (float): Factor to scale Otsu threshold.
        closing_iters (int): Number of iterations for binary closing.
        opening_iters (int): Number of iterations for binary opening.
        min_object_size_frac (float): Keep components >= this fraction of the largest component's size.
                                      1.0 to keep only the largest. 0 to disable.
        min_hole_size_frac (float): Fill holes smaller than this fraction of their parent object's size.
                                    (Applied globally based on largest remaining object). 0 to disable.

    Returns:
        np.ndarray: The generated binary foreground mask.
    """
    if image_data.ndim not in [2, 3]:
        raise ValueError("image_data must be a 2D or 3D NumPy array.")

    try:
        thresh_otsu = filters.threshold_otsu(image_data)
        threshold = thresh_otsu * otsu_scale_factor
    except Exception as e:
        print(f"Warning: Otsu thresholding failed ('{e}'). Falling back to percentile.")
        if np.any(image_data > 0):
            threshold = np.percentile(image_data[image_data > 0], 10)
        else: # Image is all zeros or non-positive
            return np.zeros_like(image_data, dtype=np.uint8) # Return empty mask

    foreground_mask = image_data > threshold

    # Morphological Post-processing - Manual Iteration
    # Default structuring element (cross for 2D, hypercross for 3D of connectivity 1) 
    # will be used by scikit-image's binary_closing/opening.
    # You can define and pass a specific `footprint` argument to skimage_binary_closing/opening if needed.
    
    if closing_iters > 0:
        current_mask = foreground_mask
        for _ in range(closing_iters):
            current_mask = skimage_binary_closing(current_mask) # Use renamed import
        foreground_mask = current_mask

    if opening_iters > 0:
        current_mask = foreground_mask
        for _ in range(opening_iters):
            current_mask = skimage_binary_opening(current_mask) # Use renamed import
        foreground_mask = current_mask
    
    if foreground_mask.any() and min_object_size_frac > 0:
        labeled_mask, num_labels = label(foreground_mask, return_num=True, connectivity=image_data.ndim)
        if num_labels > 0:
            region_props = []
            for i in range(1, num_labels + 1): # label 0 is background
                region_props.append({'label': i, 'size': np.sum(labeled_mask == i)})
            
            if region_props: 
                region_props.sort(key=lambda x: x['size'], reverse=True)
                largest_size = region_props[0]['size']
                min_size_to_keep = max(1, int(largest_size * min_object_size_frac))
                
                final_foreground_mask = np.zeros_like(foreground_mask, dtype=bool)
                for prop in region_props:
                    if prop['size'] >= min_size_to_keep:
                        final_foreground_mask[labeled_mask == prop['label']] = True
                foreground_mask = final_foreground_mask
            else: 
                 foreground_mask.fill(False)
        else: 
            foreground_mask.fill(False)

    if foreground_mask.any() and min_hole_size_frac > 0:
        labeled_mask_for_holes_ref, num_objects_ref = label(foreground_mask, return_num=True, connectivity=image_data.ndim)
        if num_objects_ref > 0:
            current_object_sizes = [np.sum(labeled_mask_for_holes_ref == i) for i in range(1, num_objects_ref + 1)]
            if current_object_sizes: 
                largest_current_object_size = max(current_object_sizes)
                area_threshold_for_holes = max(1, int(largest_current_object_size * min_hole_size_frac))
                
                if area_threshold_for_holes > 0 :
                    foreground_mask = remove_small_holes(
                        foreground_mask, 
                        area_threshold=area_threshold_for_holes, 
                        connectivity=image_data.ndim
                    )            
